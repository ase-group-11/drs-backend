...
class TestReviewCluster:
    ...
    async def test_cluster_verify_success(self, admin_client):
        ...
        mock_db.execute.side_effect = [
            make_mock_result(row=self._primary_row()),          # fetch primary
            make_mock_result(row=self._agg_row()),              # aggregate
            MagicMock(),                                        # CREATE SEQUENCE
            seq_result,                                         # nextval
            make_mock_result(row={"id": ADMIN_TEAM_ID, "full_name": "Admin User", "department": "FIRE"}),  # team info
            MagicMock(),                                        # INSERT disaster
            gate_result,                                        # gate (primary)
            MagicMock(),                                        # UPDATE report_ids[1]
            MagicMock(),                                        # UPDATE report_ids[2]
        ]
    ...
    async def test_cluster_verify_one_disaster_created(self, admin_client):
        ...
        mock_db.execute.side_effect = [
            make_mock_result(row=self._primary_row()),
            make_mock_result(row=self._agg_row()),
            MagicMock(),
            seq_result,
            make_mock_result(row={"id": ADMIN_TEAM_ID, "full_name": "Admin", "department": "FIRE"}),
            MagicMock(),   # INSERT disasters (1 call)
            gate_result,
            MagicMock(),
            MagicMock(),
        ]
