...
#             if str(unit["unit_status"]) in ("DEPLOYED", "ON_SCENE"):
#                 raise HTTPException(
#                     status_code=400,
#                     detail=f"Cannot decommission unit while {unit['unit_status']}. Complete active deployments first."
#                 )
...
#             if current_count >= unit["capacity"]:
#                 raise HTTPException(status_code=400, detail=f"Unit is at full capacity ({unit['capacity']}).")

#             # Add
#             await self.db.execute(
#                 text("INSERT INTO unit_crew (unit_id, team_member_id) VALUES (:uid, :mid)"),
