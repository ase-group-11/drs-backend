...
class UserService:
    ...
    async def verify_registration(
        ...
        try:
            ...
            # Step 5: Flush so the user row is visible within this session before
            # we refresh — but do NOT commit here. get_db() owns the commit so that
            # the full request is atomic. (FIX #9: removed explicit session.commit())
