...
class RerouteService:
    ...
    async def trigger_reroute_traffic(
        ...
        await self.mapping.highlight_alternative_routes(alternative_routes)
    ...
    # -------------------------------------------------------------------------
    # run_monitoring_cycle — called by Celery task or directly in tests
    # -------------------------------------------------------------------------

    async def run_monitoring_cycle(self, region_id: str) -> dict:
        """
        Run one monitoring cycle for an active reroute region.

        Steps:
          1. Fetch live traffic from TomTom
          2. Run dual_congestion_check (reactive + predictive)
          3. If recalculation needed:
             a. Recalculate routes via TomTom
             b. Update map
             c. Publish route.updated to RabbitMQ

        Called every 30s by Celery beat via monitor_traffic_conditions task.
        Also called directly in integration/load tests.
        """
        from app.services.predictive_congestion import dual_congestion_check
        ...
        if not check["should_recalculate"]:
            logger.debug(f"run_monitoring_cycle: no recalculation needed for {region_id}")
            return {"status": "ok", "should_recalculate": False}
        ...
        # Step 3a — recalculate
        # get_active_reroute_plan expects a disaster_id not region_id
        # Look it up from the monitoring registry via region_id
        from app.workers.tasks import _active_reroute_regions
        disaster_id = next(
            (did for did, data in _active_reroute_regions.items()
             if data.get("region_id") == region_id),
            None
        )
        if not disaster_id:
            return {"status": "ok", "should_recalculate": True, "recalculated": False}

        active_plan = await self.db.get_active_reroute_plan(disaster_id)
        if not active_plan:
            return {"status": "ok", "should_recalculate": True, "recalculated": False}
        ...
        new_routes = await self.calculate_alternative_routes(
            blocked_roads=blocked_roads,
            destinations=[{"lat": 53.3498, "lng": -6.2603}],
        )
    ...
    # -------------------------------------------------------------------------
    # handle_concurrent_incident — Phase 5 multi-incident
    # -------------------------------------------------------------------------
    ...
    async def handle_concurrent_incident(
        ...
        """
        Handle a second concurrent disaster while one is already active.

        Steps:
          1. Recompute routes considering all active incidents
          2. Reprioritize vehicles (emergency first)
          3. Update map with new combined routes
          4. Publish route.updated event

        Args:
            incident: {disaster_id, blocked_roads, severity}
        """
        disaster_id = incident.get("disaster_id", "unknown")
        blocked_roads = incident.get("blocked_roads", [])

        # Step 1 — recompute multi-incident
        result = await self.external.recompute_multi_incident_detours(
            incidents=[incident],
            vehicles=[],
        )
        ...
        # Step 2 — reprioritize (emergency vehicles get best routes)
        vehicles = await self.db.get_users_in_affected_area(disaster_id)
        if vehicles and new_routes:
            plan = self.reprioritize_flows.__func__(self, vehicles, new_routes)
            if hasattr(plan, "route_assignments"):
                route_assignments = plan.route_assignments
            else:
        ...
        await self.db.log_event(
            disaster_id=disaster_id,
            event_type="multi_incident_reroute",
            data={
                "severity": incident.get("severity"),
        ...
        logger.info(
            f"handle_concurrent_incident: disaster={disaster_id} "
            f"routes={len(new_routes)}"
        )
