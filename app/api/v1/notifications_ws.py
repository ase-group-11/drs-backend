...
IMPORTANT: location_registry functions (set_user_location, delete_user_location,
flush_offline_alerts) are SYNCHRONOUS — do not await them.
"""
...
async def broadcast_to_users(
    ...
    """
    Send a message to targeted users, or to all connected clients if
    target_user_ids is None. Silently removes dead connections.
    """
    dead: Set[str] = set()
...
async def redis_listener() -> None:
    """
    Background task: subscribes to the Redis "app_alerts" channel and
    routes each alert to the correct connected clients via broadcast_to_users().

    Reconnects automatically on transient Redis failures.
    Exits cleanly on asyncio.CancelledError (triggered by main.py shutdown).
    """
...
async def websocket_notifications(websocket: WebSocket) -> None:
    ...
    try:
        ...
        # ── Step 5: Flush alerts queued while user was offline ────────────────
        pending = flush_offline_alerts(user_id)  # sync — no await
            ...
            try:
                ...
                if frame.get("type") == "ping":
                    ...
                    if plat is not None and plon is not None:
                        set_user_location(user_id, float(plat), float(plon))  # sync — no await
            except asyncio.TimeoutError:
                # Client idle for >200 s — close cleanly
                break
