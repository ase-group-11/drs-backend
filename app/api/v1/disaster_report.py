...
def _publish_report_event(payload: dict):
    """Publish disaster.reported to RabbitMQ. Called AFTER get_db() commits."""
    try:
        from app.services.rabbitmq_service import publish_disaster_reported
        publish_disaster_reported(payload)
    except Exception as e:
        logger.error(f"Failed to publish disaster.reported event: {e}")
...
async def review_cluster(
    ...
    if event:
        background_tasks.add_task(_publish_report_event, event)
    return result
